...
import { 
    Trophy, CheckCircle, XCircle, Eye, Plus, Trash2, Edit2,
    TrendingUp, Calendar, DollarSign, FileText, AlertCircle
} from 'lucide-react';
...
const MatchManagement = () => {
    ...
    const hasAccess = checkPermission('financial_management');
    ...
    const loadMatches = async (statusFilter = null) => {
        setLoading(true);
        try {
            let query = supabase
                .from('matches')
    ...
    const totalRevenue = matches.filter(m => m.status === 'approved').reduce((sum, m) => sum + (m.actual_total_cost || 0), 0);
    const totalProfits = matches.filter(m => m.status === 'approved').reduce((sum, m) => {
        const rev = m.actual_total_cost || 0;
        const exp = m.total_expenses || 0;
        const add = m.additional_profit || 0;
        return sum + (rev - exp + add);
    }, 0);
    ...
    return (
        ...
        >
            <Helmet><title>إدارة المباريات | MTS Supreme</title></Helmet>
            
            <PageTitle title="إدارة المباريات والفعاليات" icon={Trophy} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="border-r-4 border-yellow-500">
                    <CardContent className="pt-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-sm text-gray-500">مباريات معلقة</p>
                ...
                <Card className="border-r-4 border-green-500">
                    <CardContent className="pt-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-sm text-gray-500">مباريات معتمدة</p>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <h3 className="text-lg font-bold text-blue-600">{formatCurrency(totalRevenue)}</h3>
                            </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <h3 className="text-lg font-bold text-purple-600">{formatCurrency(totalProfits)}</h3>
                            </div>
                                        ...
                                        {loading ? (
                                            ...
                                            matches.map((match) => (
                                                <tr key={match.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="p-3">{match.profiles?.name_ar}</td>
                                                    <td className="p-3">
                                                        <span className={`px-2 py-1 rounded text-xs ${
                                                            ...
                                                            {match.status === 'approved' ? 'معتمد' :
                                                             match.status === 'rejected' ? 'مرفوض' : 'معلق'}
                                                        </span>
                                                    </td>
                                                    <td className="p-3">
                                                        <Button
                                                        ...
                                                        >
                                                            ...
                                                            {match.status === 'pending' ? 'مراجعة' : 'عرض'}
                                                        </Button>
                                                    </td>
                                                </tr>
                                            ))
                ...
                <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
                    ...
                    {selectedMatch && (
                        <div className="space-y-6">
                                ...
                                <CardContent className="pt-6">
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <span className="mr-2 font-medium">{selectedMatch.profiles?.name_ar}</span>
                                        </div>
                                        <div>
                                            <span className="text-gray-500">الحالة:</span>
                                            <span className={`mr-2 px-2 py-1 rounded text-xs ${
                                                selectedMatch.status === 'approved' ? 'bg-green-100 text-green-700' :
                                                selectedMatch.status === 'rejected' ? 'bg-red-100 text-red-700' :
                                                'bg-yellow-100 text-yellow-700'
                                            }`}>
                                                {selectedMatch.status === 'approved' ? 'معتمد' :
                                                 selectedMatch.status === 'rejected' ? 'مرفوض' : 'معلق'}
                                            </span>
                                        </div>
                                    </div>
                                </CardContent>
                            ...
                            <Card className="border-2 border-blue-400 bg-blue-50">
                                <CardHeader>
                                    <CardTitle className="text-base text-blue-700 flex items-center gap-2">
                                        ...
                                        عرض السعر {selectedMatch.status === 'pending' && '(يمكنك تعديل الأسعار)'}
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                                        ...
                                                        <tr key={item.id} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="p-3">
                                                                {selectedMatch.status === 'pending' ? (
                                                                    ...
                                                                    <span className="number font-bold text-blue-600">{formatCurrency(price)}</span>
                                                                )}
                                                            </td>
                            ...
                            {/* Suppliers Cost */}
                            {(selectedMatch.status === 'pending' || selectedMatch.status === 'rejected') && (
